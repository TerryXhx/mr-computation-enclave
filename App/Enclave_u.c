...
static sgx_status_t SGX_CDECL Enclave_ocall_open_file(void* pms)
{
	...
	ms->ms_retval = ocall_open_file(ms->ms_filepath);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_close_handle(void* pms)
{
	ms_ocall_close_handle_t* ms = SGX_CAST(ms_ocall_close_handle_t*, pms);
	ocall_close_handle(ms->ms_fd);

	return SGX_SUCCESS;
}

static sgx_status_t SGX_CDECL Enclave_ocall_map_file(void* pms)
{
	ms_ocall_map_file_t* ms = SGX_CAST(ms_ocall_map_file_t*, pms);
	ms->ms_retval = ocall_map_file(ms->ms_fd, ms->ms_size);
...
static const struct {
	...
	14,
	{
		...
		(void*)Enclave_ocall_close_handle,
		(void*)Enclave_ocall_map_file,
...
{
	sgx_status_t status;
	ms_measure_enclave_t ms;
	ms.ms_dllpath = dllpath;
	ms.ms_dllpath_len = dllpath ? strlen(dllpath) + 1 : 0;
